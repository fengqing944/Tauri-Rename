...
- **系统托盘**：可选择关闭窗口时最小化到托盘，并通过托盘菜单显示或退出。
...
1. 添加一个或多个待处理文件夹。
2. 根据需要设置处理方式、分类目录、命名模板、目录映射和特殊目录。
3. 可选择补充文件，并决定是否复制；图片和 GIF 放到根目录，TXT 放到图包。
...
| 类型 | 默认目录 | 常见扩展名 |
| --- | --- | --- |
| 图片 | 图包 | jpg、jpeg、png、webp、bmp、tif、tiff、avif、heic、heif、txt |
| 视频 | 视频 | mp4、mov、mkv、avi、wmv、m4v、webm、flv、ts、m2ts |
| GIF | GIF | gif |
| 文本 | 文本 | md、nfo |

TXT 会自动进入图包，并作为水印/说明文件保留原名，不参与重命名；MD、NFO 分类可在界面中开关控制。

## 处理方式

| 方式 | 行为 |
| --- | --- |
| 标准整理 | 执行目录映射、创建分类目录、移动分类、复制补充文件，并按规则重命名。 |
| 只重命名，不移动 | 不执行目录映射，不创建分类目录，不移动文件，不复制补充文件，只在原位置重命名支持的媒体文件。 |
